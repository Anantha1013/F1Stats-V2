...
response=urlopen(url)
telemetry_data=json.loads(response.read().decode('utf-8')) #updates -> every 4 minutes 
print(telemetry_data[0])

#TODO- simulate the race using the telemetry obtained



#intervals
url=f'https://api.openf1.org/v1/intervals?session_key={session_key}&driver_number=1'
response=urlopen(url)
